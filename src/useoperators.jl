      ...
      for i = 1:sbpface.numnodes
        ...
        for j = 1:sbpface.stencilsize
          ...
          uface[i,1,findex] += sbpface.interp[j,i]*uvol[sbpface.perm[j,face.faceL],
                                                        face.elementL]
          uface[i,2,findex] += sbpface.interp[j,i]*uvol[sbpface.perm[j,face.faceR],
                                                        face.elementR]
        end
      end
    end
  ...
  @inbounds begin
    for (findex, face) in enumerate(ifaces)
      for i = 1:sbpface.numnodes
        ...
        uface[:,i,2,findex] = zeros(Tsol, size(uvol,1))
        for j = 1:sbpface.stencilsize
          for field = 1:size(uvol,1)
            ...
            uface[field,i,2,findex] += sbpface.interp[j,i]*
      ...
      for i = 1:sbpface.numnodes
        ...
        for j = 1:sbpface.stencilsize
          ...
          res[sbpface.perm[j,face.faceR],face.elementR] -=
            sbpface.interp[j,iR]*sbpface.wface[iR]*flux[iR,findex]
        end
      end
    end
  end
