    ...
    @eval begin
      @testset "Testing mappingjacobian! ($(string($TSBP)) method)" begin
        ...
        for p = 1:4
          ...
          mappingjacobian!(sbp, x, dξdx, jac)
          # dxi/dx = (1,0,0)
          @test ≈(vec(dξdx[1,1,:,1]), ones(sbp.numnodes), atol=1e-11)
          @test ≈(vec(dξdx[1,2,:,1]), zeros(sbp.numnodes), atol=1e-11)
          @test ≈(vec(dξdx[1,3,:,1]), zeros(sbp.numnodes), atol=1e-11)
          # deta/dx = (0,1,0)
          @test ≈(vec(dξdx[2,1,:,1]), zeros(sbp.numnodes), atol=1e-11)
          @test ≈(vec(dξdx[2,2,:,1]), ones(sbp.numnodes), atol=1e-11)
          @test ≈(vec(dξdx[2,3,:,1]), zeros(sbp.numnodes), atol=1e-11)
          # dzeta/dx = (0,1,0)
          @test ≈(vec(dξdx[3,1,:,1]), zeros(sbp.numnodes), atol=1e-11)
          @test ≈(vec(dξdx[3,2,:,1]), zeros(sbp.numnodes), atol=1e-11)
          @test ≈(vec(dξdx[3,3,:,1]), ones(sbp.numnodes), atol=1e-11)
          # jac = 1
          @test ≈(vec(jac[:,1]), ones(sbp.numnodes), atol=1e-11)
        end
      end
    end
  end
