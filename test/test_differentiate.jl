facts("Testing SummationByParts Module (differentiate methods)...") do
  ...
  context("Testing differentiate! (TriSBP, scalar field method)") do
  ...
  context("Testing differentiate! (TetSBP, scalar field method)") do
    # verify the accuracy of the differentiation operators
    for d = 1:4
  ...
  context("Testing differentiate! (TriSBP, vector field method)") do
  ...
  context("Testing differentiate! (TetSBP, vector field method)") do
  ...
  context("Testing differentiateElement! (TriSBP, scalar field method)") do
  ...
  context("Testing differentiateElement! (TetSBP, scalar field method)") do
    # verify the accuracy of the differentiation operators
    for d = 1:4
      sbp = TetSBP{Float64}(degree=d)
      cub, vtx = tetcubature(2*d-1, Float64)
  ...
  context("Testing differentiateElement! (TriSBP, vector field method)") do
    # build a two element grid, and verify that Dxi*x = 0.5 or 0, depending on
    # orientation of local coordinates
    for p = 1:4
      sbp = TriSBP{Float64}(degree=p)
  ...
  context("Testing differentiateElement! (TetSBP, vector field method)") do
    # build a single element grid, and verify that Dxi * x = 0.5 or 0, depending
    # on orientation of local coordinates
    for p = 1:4
      sbp = TetSBP{Float64}(degree=p)
