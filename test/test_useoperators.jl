facts("Testing SummationByParts Module (useoperators.jl file)...") do
  ...
  context("Testing SummationByParts.mappingjacobian! (TetSBP method)") do
    ...
    for p = 1:4
      ...
      mappingjacobian!(sbp, x, dξdx, jac)
      # dxi/dx = (1,0,0)
      @fact dξdx[1,1,:,1] => roughly(ones(1,1,sbp.numnodes), atol=1e-12)
      @fact dξdx[1,2,:,1] => roughly(zeros(1,1,sbp.numnodes), atol=1e-12)
      @fact dξdx[1,3,:,1] => roughly(zeros(1,1,sbp.numnodes), atol=1e-12)
      # deta/dx = (0,1,0)
      @fact dξdx[2,1,:,1] => roughly(zeros(1,1,sbp.numnodes), atol=1e-12)
      @fact dξdx[2,2,:,1] => roughly(ones(1,1,sbp.numnodes), atol=1e-12)
      @fact dξdx[2,3,:,1] => roughly(zeros(1,1,sbp.numnodes), atol=1e-12)
      # dzeta/dx = (0,1,0)
      @fact dξdx[3,1,:,1] => roughly(zeros(1,1,sbp.numnodes), atol=1e-12)
      @fact dξdx[3,2,:,1] => roughly(zeros(1,1,sbp.numnodes), atol=1e-12)
      @fact dξdx[3,3,:,1] => roughly(ones(1,1,sbp.numnodes), atol=1e-12)
      # jac = 1
