@testset "Testing OrthoPoly Module..." begin

  for T = (Float32, Float64, ComplexF64)
    @eval begin
      @testset "Testing OrthoPoly.lglnodes for DataType $(string($T))" begin
        x, w = OrthoPoly.lglnodes(2, ($T))
        @test ≈(x, ($T)[-1, 0, 1]; atol=10*eps(typeof(real(one($T)))) )
        @test ≈(w, ($T)[1/3, 4/3, 1/3];
                            atol=10*eps(typeof(real(one($T)))) )
        x, w = OrthoPoly.lglnodes(3, ($T))
        @test ≈(x, ($T)[-1, -1/sqrt(5), 1/sqrt(5), 1];
                            atol=10*eps(typeof(real(one($T)))) )
        @test ≈(w, ($T)[1/6, 5/6, 5/6, 1/6];
                            atol=10*eps(typeof(real(one($T)))) )
        x, w = OrthoPoly.lglnodes(4, ($T))
        @test ≈(x, ($T)[-1, -sqrt(3/7), 0, sqrt(3/7), 1];
                            atol=10*eps(typeof(real(one($T)))) )
        @test ≈(w, ($T)[1/10, 49/90, 32/45, 49/90, 1/10];
                            atol=10*eps(typeof(real(one($T)))) )
      end
    end
  end
  ...
  for T = (Float32, Float64, ComplexF64)
    @eval begin
      @testset "Testing OrthoPoly.lgnodes for DataType $(string($T))" begin
        x, w = OrthoPoly.lgnodes(2, ($T))
        @test ≈(x, ($T)[-1/sqrt(3), 1/sqrt(3)];
                            atol=100*eps(typeof(real(one($T)))) )
        @test ≈(w, ($T)[1, 1];
                            atol=100*eps(typeof(real(one($T)))) )
        x, w = OrthoPoly.lgnodes(3, ($T))
        @test ≈(x, ($T)[-sqrt(3/5), 0, sqrt(3/5)];
                            atol=100*eps(typeof(real(one($T)))) )
        @test ≈(w, ($T)[5/9, 8/9, 5/9];
                            atol=100*eps(typeof(real(one($T)))) )
        x, w = OrthoPoly.lgnodes(4, ($T))
        @test ≈(x, ($T)[-sqrt(3/7 + (2/7)*sqrt(6/5)),
                                 -sqrt(3/7 - (2/7)*sqrt(6/5)),
                                 sqrt(3/7 - (2/7)*sqrt(6/5)),
                                 sqrt(3/7 + (2/7)*sqrt(6/5))];
                                 atol=100*eps(typeof(real(one($T)))) )
        @test ≈(w, ($T)[(18-sqrt(30))/36, (18+sqrt(30))/36,
  ...
  for T = (Float32, Float64, ComplexF64)
    @eval begin
      @testset "Testing OrthoPoly.jacobipoly for DataType $(string($T))" begin
        # compare against 5th degree Legendre polynomial
        x = ($T)[-1:0.1:1;]
        L = OrthoPoly.jacobipoly(x, 0.0, 0.0, 5)
        L ./= L[21] # necessary for standardization
        @test ≈(L, (63.0 .*x.^5 -70.0 .*x.^3 + 15.0 .*x)/8; atol=10*eps(typeof(real(one($T)))) )
      end
    end
  end

  @eval begin
    @testset "Testing OrthoPoly.diffjacobipoly" begin
      alpha = 0.3
      beta = 0.65
      x = Float64[-1:0.1:1;]
      eps_step = 1e-60
      for N = 1:10
        ...
        @test ≈(dP, dP_cmplx; atol=1e-13)
      end
    end
  end

  @eval begin
    @testset "Testing OrthoPoly.proriolpoly (Triangle method)" begin
      ...
      @test ≈(innerprod, I(N); atol=1e-14)
    end
  end

  @eval begin
    @testset "Testing OrthoPoly.proriolpoly (Tetrahedron method)" begin
      # Integrate the product of the polynomials, and verify that they are
      # orthonormal
  ...
  @eval begin
    @testset "Testing OrthoPoly.diffproriolpoly" begin
      ...
      for r = 0:q
        for j = 0:r
          ...
          @test ≈(dPdy, -imag(Pc)/eps_step; atol=1e-12)
          xc .-= eps_step*im
          Pc = OrthoPoly.proriolpoly(xc, yc, i, j)
          @test ≈(dPdx, -imag(Pc)/eps_step - dPdy; atol=1e-12)
        end
      end
    end
  end
