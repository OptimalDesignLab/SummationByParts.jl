facts("Testing SummationByParts Module (Jacobian of weak differentiate methods)...") do
 ...
 for TSBP = ((getLineSegSBPLobbato,1), (getLineSegSBPLegendre,1),
    ...
    @eval begin
      context("Testing weakDifferentiateElement_jac! ("string($TSBP[1])" scalar field method)") do
        for p = 1:4
          for trans in [true, false]
            ...
            for di = 1:size(sbp.Q,3)
              weakDifferentiateElement_jac!(sbp, di, flux_jac, dRdu, Add(),
                                            trans)
            end
  ...
  for TSBP = ((getLineSegSBPLobbato,1), (getLineSegSBPLegendre,1),
              ...
              (getTetSBPGamma,3), (getTetSBPOmega,3), (getTetSBPDiagE,3))
    @eval begin
      context("Testing weakDifferentiateElement_jac! ("string($TSBP[1])" vector field method)") do
        for p = 1:4
          for trans in [true, false]
            sbp = ($TSBP[1])(degree=p)
